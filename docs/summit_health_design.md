# Summit Health Care - System Design

Hospitals
Departments
Pharmacy
Patient population
Operational workflow
Medication workflow
Seasonality
Volumes
Staffing


Name: Summit Health Care
Type: Regional Health System
Facilities: 
    - Summit Medical Center (Level I Trauma / Academic Medical Center) - 550 beds
    - Summit Community Hospital - 150 beds
    - Summit Children's Hospital - 250 beds
    - Summit Outpatient Infusion Center - 10 chairs
    - Summit Specialty Pharmacy
    - Summit Outpatient Pharmacy

## Pharmacy Department
Summit Health Care has 1 centralized inpatient pharmacy supporting all hospital locations, 1 outpatient pharmacy, and 1 specialty pharmacy.

Pharmacy operations include:
    - Central pharmacy
    - IV sterile compounding
    - Decentral clinical pharmacy (ICU, medsurg, emergency department, oncology)
    - Automated dispensing cabinets (Omnicell)
    - Medication safety services
    - Controlled substance management
    - Pharmacy purchasing


How many hospitals?
How many beds?
Exactly which departments?
How many medication orders per day?
Which medications should exist?
Which provider specialties?
Which pharmacy workflows?
How does a Monday differ from a Saturday?
What does flu season look like?
What makes Summit Health unique?